...
Strict sync builds one preflight plan and refuses to begin while any valid registered worktree contains an operator-owned merge, revert, cherry-pick, rebase, apply-mailbox, bisect, or sequencer operation. It resolves the exact per-worktree Git administrative paths (`MERGE_HEAD`, `REVERT_HEAD`, `CHERRY_PICK_HEAD`, `rebase-merge`, `rebase-apply`, `BISECT_START`, and `sequencer`) instead of resolving ambiguous revisions, so ordinary branches or tags with those names do not impersonate operation state. Present commit markers must contain canonical commit identifiers, administrative directories must have the expected kind, and every inspection failure stops sync. Rejection occurs before fetch, stash, checkout/worktree changes, index reset, LLM dispatch, commit, or push and reports the matching Git continue, abort, quit, or reset commands.
...
When the target branch is held by a linked worktree, sync first prunes stale registration and preserves sibling changes before retrying the switch. Sibling adoption may commit locally to release the checkout, but it does not publish from the sibling; the normal target-branch push remains the publication boundary. Before its first local mutation, the strict-sync transaction snapshots the caller and target sibling checkout, commit, index, tracked contents, untracked contents, stash list, and topology. A pre-publication failure resets gix-created commits and index changes, restores the exact files and staged/unstaged distinction, and reattaches a detached main worktree or recreates a removed linked worktree at its original path. A failure after a successful push cannot undo the remote write: Gix preserves the published checkout and any invocation-owned recovery stash, emits `SYNC_SWITCH_HANDOFF`, and never reports `SYNCED`.
