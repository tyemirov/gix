...
An explicit ordinary branch target receives the pending files and their commits. An unprotected default target receives those commits and publishes directly. For a protected default target, sync publishes local commits and pending files on a generated review branch. Sync opens a pull request from that branch against the resolved default branch. The review branch stays active. The local default branch keeps its original commits.

Sync gets the default branch name from the remote symbolic `HEAD`. Names such as `main`, `master`, `qqq`, and `default` have no special function. GitHub supplies the branch protection state. A failed protection lookup stops sync before it generates dirty commits. With no local commits to publish, sync merges the remote counterpart without a push.
