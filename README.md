...
Gix constructs a merge plan from exact BASE-to-OURS and BASE-to-THEIRS line changes. Independent changes remain fixed. Each overlapping block requires an explicit decision: select OURS, select THEIRS, or combine both sources. Source selections preserve every byte. A combination can contain compatible requirements from both issue records. Word counts and issue status do not establish authority.

Each decision records which source changes it retains, combines, or supersedes. Every candidate for a two-sided conflict requires a separate semantic audit. The audit uses the original sources and the recorded decisions. A rejection supplies a reason for repair. A repeated rejected candidate stops the operation.

Read context and edit scope are separate. Small files supply complete source snapshots. Large files supply complete affected issue records or surrounding lines. An ambiguous source location supplies the complete file. The model can request complete file context once. It can also report unresolved intent. These outcomes have distinct errors.

Before staging, Gix parses complete Go, JSON, and YAML files and checks issue identifiers for duplicates. Before commit, Gix requires no unmerged paths and checks the staged index against both merge parents. A path passes when Git reports no new whitespace error relative to one parent. Syntax and source accounting do not prove semantic correctness.

Each region permits four provider requests across decisions, audits, and context expansion. A provider round with no response stops semantic repair. Unresolved intent, unavailable context, exhausted requests, invalid file syntax, and local failures stop before publication. The existing transaction restores the caller state before publication. After publication, recovery preserves the published state through an explicit handoff.

See [the conflict contract](docs/merge-conflicts.md) for the protocol, acceptance boundaries, and separate live-provider evaluation.
...
Use the reusable LLM client (`github.com/tyemirov/utils/llm`) to summarize staged changes or recent history. `gix sync` uses the configured provider order for semantic decisions and audits during strict sync. For semantic resolution, `timeout_seconds` is the request budget for each provider. Each candidate or audit request can use one complete provider round. A provider round with no response stops semantic repair and starts rollback. A transport or authentication error never becomes model feedback. Returned decisions, audit results, or context requests control the next of four bounded requests. Gix reports the active region, strategy, attempt, and deadline.
