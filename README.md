...
Strict sync refuses to begin while `REVERT_HEAD` identifies an unfinished operator-owned `git revert`. Gix checks this boundary before fetch, stash, branch/worktree changes, index reset, LLM dispatch, commit, or push, and leaves the commit, index, worktree, untracked files, and revert state unchanged. Resolve the transaction explicitly with `git revert --continue`, cancel it with `git revert --abort`, or retain its current changes while ending the sequencer with `git revert --quit`, then retry sync.
...
When the target branch is held by a linked worktree, sync first prunes stale worktree registration and preserves any sibling changes that need to survive before retrying the switch. It does not discard dirty sibling work as a shortcut to changing branches. A successful explicit-target sync leaves the requested branch active. If a later strict-sync step fails, Gix restores the starting checkout and the valid worktree topology it captured before mutation, including reattaching a detached main worktree or recreating a removed linked worktree at its original path.
