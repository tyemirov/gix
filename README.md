...
| `gix sync <default-branch>` | Merge remote commits. Publish new work directly for an unprotected default, or through a review branch for a protected default. |
...
An explicit ordinary branch target receives the pending files and their commits. An unprotected default target receives those commits and publishes directly. For a protected default target, sync publishes local commits and pending files on a generated review branch. Sync opens a pull request from that branch against the resolved default branch. The review branch stays active. The local default branch keeps its original commits until a merged review contains them.

Sync uses an open review again when that review contains the same local default commit. After a squash merge, sync updates the local default to the remote default commit. Sync keeps newer local commits for a separate review.

Sync gets the default branch name from the remote symbolic `HEAD`. Names such as `main`, `master`, `qqq`, and `default` have no special function. The selected remote supplies the repository identity for GitHub branch protection and pull requests. A failed protection lookup stops sync before it generates dirty commits. With no local commits to publish, sync uses a fast-forward update without a push.
