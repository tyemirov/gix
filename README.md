...
- `capture: { variable: "<name>", value: branch|commit }` — record the current branch or HEAD commit into a workflow variable so later steps can restore it.
- `restore: { variable: "<name>", value: branch|commit }` — jump back to a previously captured branch/commit. Validation fails if the capture name is missing, and `value` defaults to the original capture kind when omitted.
