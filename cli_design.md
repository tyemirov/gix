...
| Side effects | Creates or updates the target branch, edits workflow files, updates GitHub Pages, and changes the default branch. Closes a pull request only when its head repository and head branch match the target repository and branch. Changes the base of other pull requests. The direct command fetches the remote source and target before the deletion safety check. The delete request includes the verified source commit. Git rejects deletion if the source changes. The direct command deletes the old default branch after all safety gates pass. Reusable workflows delete it only when `delete_source_branch` is `true`. |
| Outputs | Logs the promoted source and target branches. Reports the source branch safety result and deletion result. |
