...
func TestSyncProviderRoundFailureStopsSemanticRepairAndRestoresTarget(testInstance *testing.T) {
	...
	const (
		baseBranchName   = "feature/provider-failure-base"
		targetBranchName = "bugfix/provider-failure-target"
		conflictedPath   = "CHANGELOG.md"
	)
	...
	require.NoError(testInstance, os.WriteFile(conflictedFilePath, []byte(initialContent), 0o644))
	runGit(testInstance, repositoryPath, "add", conflictedPath)
	runGit(testInstance, repositoryPath, "commit", "-m", "seed provider failure fixture")
	runGit(testInstance, repositoryPath, "push", "-u", "origin", "master")

	runGit(testInstance, repositoryPath, "switch", "-c", baseBranchName)
	require.NoError(testInstance, os.WriteFile(conflictedFilePath, []byte(incomingContent), 0o644))
	runGit(testInstance, repositoryPath, "add", conflictedPath)
	runGit(testInstance, repositoryPath, "commit", "-m", "update incoming provider failure note")
	runGit(testInstance, repositoryPath, "push", "-u", "origin", baseBranchName)
	...
	require.NoError(testInstance, os.WriteFile(conflictedFilePath, []byte(targetContent), 0o644))
	runGit(testInstance, repositoryPath, "add", conflictedPath)
	runGit(testInstance, repositoryPath, "commit", "-m", "update target provider failure note")
	runGit(testInstance, repositoryPath, "push", "-u", "origin", targetBranchName)
	targetCommit := strings.TrimSpace(runGit(testInstance, repositoryPath, "rev-parse", "HEAD"))
	...
	proxyServer := httptest.NewServer(http.HandlerFunc(func(responseWriter http.ResponseWriter, request *http.Request) {
		...
		_, _ = fmt.Fprint(responseWriter, "rejected secret must not enter model feedback")
	}))
	testInstance.Cleanup(proxyServer.Close)

	var openAIRequestCount atomic.Int64
	openAIServer := httptest.NewServer(http.HandlerFunc(func(responseWriter http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/chat/completions" {
			http.NotFound(responseWriter, request)
			return
		}
		openAIRequestCount.Add(1)
		responseWriter.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(responseWriter, `{"choices":[{"finish_reason":"length","message":{"content":""}}]}`)
	}))
...
operations:
  - command: ["sync"]
    with:
      remote: origin
      require_clean: true
`, openAIServer.URL, proxyServer.URL)
	require.NoError(testInstance, os.WriteFile(configurationPath, []byte(configurationContent), 0o600))
	...
	require.Error(testInstance, runError)
	require.Contains(testInstance, output, "AI_MERGE_ROLLBACK")
	require.Contains(testInstance, output, "status=403")
	require.Contains(testInstance, output, "openai empty-response recovery failed")
	require.Contains(testInstance, output, "stopping semantic repair")
	require.NotContains(testInstance, output, "rejected secret must not enter model feedback")
	require.NotContains(testInstance, output, "requesting validation-guided repair")
	require.NotContains(testInstance, output, "AI_MERGE_HANDOFF")
	require.Equal(testInstance, int64(1), proxyRequestCount.Load())
	require.Equal(testInstance, int64(2), openAIRequestCount.Load())
