...
func TestSyncHelpDescribesMissingBranchCurrentHeadContract(testInstance *testing.T) {
	...
	require.Contains(testInstance, output, "Before fetch or mutation, strict sync resolves exact per-worktree Git administrative paths and rejects operator-owned merge, revert, cherry-pick, rebase, apply-mailbox, bisect, or sequencer state; ordinary refs with those names do not count.")
	require.Contains(testInstance, output, "The strict-sync transaction snapshots the caller and target sibling checkout, commit, index, tracked files, untracked files, stashes, and topology.")
	require.Contains(testInstance, output, "A failure before push restores that exact state; a failure after push preserves the published recovery state and reports a handoff.")
