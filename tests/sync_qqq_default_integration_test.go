...
func TestSyncExplicitQQQDefaultFromCheckout(testInstance *testing.T) {
	...
	for _, protected := range []bool{false, true} {
		for _, startingBranch := range []string{defaultBranch, "main", "master"} {
			testInstance.Run(fmt.Sprintf("from_%s_protected_%t", startingBranch, protected), func(testInstance *testing.T) {
				fixture := newProtectedSyncFixture(testInstance, defaultBranch)
