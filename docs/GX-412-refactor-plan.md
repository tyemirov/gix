...
## 1. Duplicate preset CLI plumbing (edge validation drift)
...
## 2. Untyped preset mutation via `map[string]any`
...
## 3. Domain option structs still expose raw primitives
...
## 4. Taskrunner dependency resolver conflates responsibilities
...
## 5. Missing negative-path tests for CLI file IO
