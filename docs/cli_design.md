...
| Side effects | Creates or updates the target branch, edits workflow files, updates GitHub Pages, and changes the default branch. Closes a pull request only when its head repository and head branch match the target repository and branch. Changes the base of other pull requests. The direct command deletes the old default branch after all safety gates pass. Reusable workflows delete it only when `delete_source_branch` is `true`. |
