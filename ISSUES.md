...
- [x] [GX-222] Rename branch-default to default and update dependent workflow plumbing
  ...
  - Resolution: Promoted `gix default` as the canonical command, added `branch-default` as a deprecated alias with warnings, refreshed default configs/docs/tests/workflow command keys, and taught the workflow builder plus migrate CLI to accept the new and legacy names.
...
- [x] [GX-227] Nest repo-history-remove under files rm
  ...
  - Resolution: Mounted the history purge command under `files rm`, refreshed defaults/docs/tests to the new path, and left `gix rm`/`repo rm` aliases in place with migration warnings.
