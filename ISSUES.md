...
- [x] [GX-419] Cleanup workflow replaces Go imports via `files.apply` `path="**/*.go"`, which is treated as a literal path and creates directories named `**` with files `*.go`. — Moved glob-based replacements to a `repo.files.replace` action so patterns are handled correctly and literal files are no longer created.
