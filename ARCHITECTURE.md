...
Before strict sync starts its own transaction, `strict_sync_preflight.go` runs `git worktree repair` from the caller repository and re-lists the repaired topology. Git rewrites only canonical registration links, reconnecting existing linked checkouts after the primary repository moves; a missing checkout remains prunable, and repair never removes a live directory. A repair failure stops with repository context. Preflight then lists every valid registered worktree and resolves the exact per-worktree paths for `MERGE_HEAD`, `REVERT_HEAD`, `CHERRY_PICK_HEAD`, `rebase-merge`, `rebase-apply`, `BISECT_START`, and `sequencer`. Absence is the only idle administrative state. Present commit files must contain canonical commit identifiers, administrative directories must have the expected filesystem kind, and path, read, or validation failures stop sync. The same preflight inspects each index for unmerged entries, catching operator-owned conflicts such as a failed stash apply even when no administrative marker exists. This exact-path contract ignores ordinary branches or tags with marker-like names while detecting operator-owned merge, revert, cherry-pick, rebase, apply-mailbox, bisect, multi-command sequencer, and unmerged-index state in both the caller and adoptable siblings. After registration repair, preflight finishes before fetch, LLM access, or content, index, ref, checkout, or topology mutation and returns explicit recovery guidance.
