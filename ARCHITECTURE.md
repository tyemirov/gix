...
The repository release lifecycle separates local sealing, remote publication, and Pages activation. The `make release`, `make publish`, and `make deploy` targets accept no release inputs. Each repository declares `semver` or `calver` in `.mprlab/release.yml`. `scripts/release/prepare_release.sh` calls `gix release next` once and uses its version decision. Gix uses resolved commit IDs for all SemVer evidence. Gix divides complete commit messages, diff summaries, and Unreleased notes into bounded evidence packets. A repository can also declare one root Go install channel. That channel adds a deterministic v1 transport successor, module path, and product-version file to the same decision.

The LLM classifies every evidence packet. Gix uses the highest required bump from all packet results. The first packet also contains the bounded diff excerpt. Conventional Commit breaking markers set a `major` floor. A `feat` marker sets a `minor` floor. The LLM can increase the floor but cannot decrease it.

Invalid or unavailable output stops the release. An untagged SemVer repository starts at `v1.0.0`. CalVer uses the canonical UTC timestamp. `prepare_release.sh` records immutable source and release commits with payload hashes under `.git/mprlab-release`. For Gix, it updates the embedded product-version file and creates an annotated product tag plus an annotated root-module transport tag on the release commit. `publish_release.sh` publishes the sealed refs and assets only after it validates both tags. `deploy_pages_artifact.sh` validates the published manifest and Pages archive before it changes the fixed `gh-pages` target.

New release preparation uses a separate candidate receipt. It promotes the receipt only after complete verification. A failure preserves the prior receipt. If sealing fails after commit or tag creation, a compare-and-swap ref transaction restores the prepared source and deletes both transaction-owned tags. At an exact release tag, preparation becomes an idempotent reconciliation. It reuses a complete local receipt or reconstructs it from the matching GitHub Release.

Receipt reconstruction validates the manifest, notes, payload hashes, remote product and transport tags, source parent, and exact release metadata files. The manifest's `go_install` object binds the root module path and transport version to the product version and release commit.
