...
Before strict sync starts its own transaction, `strict_sync_preflight.go` verifies that `REVERT_HEAD` is absent. A pre-existing revert is operator-owned, including when every conflict is resolved and only `git revert --continue` remains. The preflight fails before fetch or mutation and names the explicit continue, abort, and quit choices; an inspection failure also stops sync rather than being interpreted as an absent revert. This keeps dirty-sync clustering from resetting or repartitioning the sequencer-owned index.

The enclosing strict-sync transaction snapshots the starting checkout plus every valid registered worktree before any target switch or sibling adoption. Successful explicit-target sync retains the requested checkout. On failure, a cancellation-independent bounded cleanup first restores the starting named branch or detached commit, then reattaches a main worktree that adoption detached or recreates a removed linked worktree at its original path. A stashed caller worktree is restored only after that checkout rollback succeeds. Cleanup reports `SYNC_SWITCH_ROLLBACK`; a failed restoration retains both the sync and cleanup errors under `SYNC_SWITCH_HANDOFF`.
