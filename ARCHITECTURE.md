...
The repository release lifecycle separates local sealing, remote publication, and Pages activation. The `make release`, `make publish`, and `make deploy` targets accept no release inputs. Each repository declares `semver` or `calver` in `.mprlab/release.yml`. A SemVer repository can declare one positive `semver.fixed_major` value. Gix declares `fixed_major: 1`; other repositories retain standard SemVer unless they declare their own policy after analysis. `scripts/release/prepare_release.sh` calls `gix release next` once and uses its version decision. Gix uses resolved commit IDs for all SemVer evidence. Gix divides complete commit messages, diff summaries, and range-scoped changelog changes into bounded evidence packets.
