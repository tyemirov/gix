...
`merge_conflict_strategy.go` derives token edits from the common base. Small conflicts use a bounded matrix. Large conflicts use the linear-memory algorithm. Both algorithms require the same deletion-intent proof. A semantic correction must preserve each compatible deletion and addition. A complete copy of both sides cannot bypass deletion validation. Backtick code spans stay in one token to keep prose changes outside code. Source containment can prove replacement text, but each deletion requires a separate check.
