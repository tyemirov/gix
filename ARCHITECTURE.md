...
The repository release lifecycle separates local sealing, remote publication, and Pages activation. The `make release`, `make publish`, and `make deploy` targets accept no release inputs. The release helper supplies one typed `semver` or `calver` policy when it calls `gix release next`. A SemVer invocation can include one positive fixed major. The Gix helper supplies fixed major `1`. Gix does not read MPR Lab repository files. The version decision records the complete applied policy in contract v2. Gix uses resolved commit IDs for all SemVer evidence. Gix divides complete commit messages, diff summaries, and range-scoped changelog changes into bounded evidence packets.
...
Invalid or unavailable output stops the release. An untagged SemVer repository starts at `v1.0.0`. CalVer uses the canonical UTC timestamp. `prepare_release.sh` records immutable source and release commits with payload hashes under `.git/mprlab-release`. It creates one annotated version tag on the release commit. `publish_release.sh` publishes the sealed ref and assets only after it validates that tag. `deploy_pages_artifact.sh` validates the published manifest and Pages archive before it changes the fixed `gh-pages` target.

New release preparation uses a separate candidate receipt. It promotes the receipt only after complete verification. A failure preserves the prior receipt. If sealing fails after commit or tag creation, a compare-and-swap ref transaction restores the prepared source and deletes the transaction-owned tag. At an exact release tag, preparation becomes an idempotent reconciliation. It reuses a complete local receipt or reconstructs it from the matching GitHub Release.

Receipt reconstruction validates the manifest, notes, payload hashes, remote version tag, source parent, and exact release metadata files. The decision, tag, manifest, binary, and GitHub Release use one authoritative version.
...
Resolution uses a ranked fidelity ladder. Byte-identical sides and unilateral changes resolve directly because they contain no two-sided semantic choice. Marker-free conflicts retain their exact current-stage decision. Every marker-bearing region changed by both sides requires semantic LLM audit. When one insertion word sequence occurs in order in the other insertion, Gix starts from the side with the complete sequence. When both word sequences are identical, Gix starts from OURS. Validation requires that exact sequence and rejects missing, reordered, added, or duplicated word tokens. Other empty-BASE concurrent insertions start from exact OURS-then-THEIRS content. `merge_conflict_strategy.go` tokenizes other unresolved regions without a byte change. It derives BASE-to-side edits with a bounded longest-common-subsequence matrix. It combines only non-overlapping edit spans into a candidate. Local strategies construct high-fidelity proposals. They do not make the final semantic decision.

Every two-sided marker-bearing region reaches the model, but untouched file content does not. Each candidate is region-scoped and sentinel-wrapped so transport trimming cannot change boundary whitespace. Hard validation rejects conflict markers, exact BASE content, incomplete disjoint insertions, and invalid complete overlap sequences. Local token validation proves exact replacement intent when possible. A candidate then enters a separate semantic-auditor request. An approval accepts a locally validated candidate. A locally valid audit correction completes immediately. If replacement-intent proof is unavailable, the resolver retains the structurally valid correction for the next repair request. An approval cannot accept that candidate. Only a later locally valid correction completes it. A genuinely overlapping region without a safe local candidate starts with model generation under the same contract. Responses that fail hard validation supply feedback for the next of four bounded attempts.
