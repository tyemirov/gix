...
Each text conflict with changes on both sides requires a semantic audit. Complete issue insertions use their canonical identifiers as record keys. Each distinct identifier must appear once in the result. A shared identifier permits an exact source record, subject to the complete-sequence rule. The model returns one source selection key per identifier. Gix constructs the result from the source records after identifier changes, in canonical order. Unknown keys, duplicate identifiers, missing selections, and rewritten records fail validation. Independent records remain present. Partial records and other text use the general conflict rules.
...
`merge_conflict_strategy.go` derives token edits from the common base. Small conflicts use a bounded matrix. Large conflicts use the linear-memory algorithm. Both algorithms require the same deletion-intent proof. A semantic correction must preserve each compatible deletion and addition. A complete copy of both sides cannot bypass deletion validation.
