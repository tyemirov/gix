...
The repository-owned release lifecycle separates local sealing, remote publication, and Pages activation. `scripts/release/prepare_release.sh` records immutable source and release commits plus payload hashes under `.git/mprlab-release`; `publish_release.sh` publishes those exact refs and assets; `deploy_pages_artifact.sh` downloads and revalidates the published manifest and Pages archive before touching `gh-pages`. A new SemVer release requires exactly one explicit bump or exact-version intent from the Make boundary; missing or conflicting intent stops before CI, and a SemVer bump is invalid for timestamp-derived CalVer. New release preparation writes to a sibling candidate receipt and promotes it only after complete verification, preserving the prior canonical receipt on failure. If sealing fails after the changelog commit or annotated tag is created, a compare-and-swap ref transaction removes only that release-owned tag and restores the default branch and `CHANGELOG.md` to the prepared source. When `HEAD` is an exact release tag, preparation becomes an idempotent zero-intent reconciliation: it reuses a verified complete local receipt or reconstructs a missing or incomplete receipt from the matching published GitHub Release after verifying the manifest identity, notes, payload hashes, remote annotated tag, source parent, and changelog-only release commit.
