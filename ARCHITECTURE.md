...
The repository-owned release lifecycle separates local sealing, remote publication, and Pages activation. The `make release`, `make publish`, and `make deploy` boundaries have no release inputs. `scripts/release/prepare_release.sh` detects the tag scheme, records immutable source and release commits plus payload hashes under `.git/mprlab-release`, and invokes the hidden `message semver` decision node for an established SemVer line. That node sends the committed range's messages, diff summary, Unreleased changelog, and bounded diff excerpt through the configured LLM connection policy. Its closed output is `major`, `minor`, or `patch`. Conventional Commit breaking markers enforce a `major` floor and `feat` enforces a `minor` floor; the LLM can elevate but cannot lower the floor. Invalid or unavailable output fails closed. An untagged repository starts at `v1.0.0`, while an established CalVer line remains timestamp-derived. `publish_release.sh` publishes the exact sealed refs and assets; `deploy_pages_artifact.sh` downloads and revalidates the published manifest and Pages archive before touching the fixed `gh-pages` target. New release preparation writes to a sibling candidate receipt and promotes it only after complete verification, preserving the prior canonical receipt on failure. If sealing fails after the changelog commit or annotated tag is created, a compare-and-swap ref transaction removes only that release-owned tag and restores the default branch and `CHANGELOG.md` to the prepared source. When `HEAD` is an exact release tag, preparation becomes an idempotent reconciliation: it reuses a verified complete local receipt or reconstructs a missing or incomplete receipt from the matching published GitHub Release after verifying the manifest identity, notes, payload hashes, remote annotated tag, source parent, and changelog-only release commit.
