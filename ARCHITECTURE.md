...
The LLM creates one public-contract candidate for each evidence packet. A second model call audits the candidate against the same packet. Gix maps incompatible, additive, and compatible public effects to `major`, `minor`, and `patch`. The first packet also contains the bounded diff excerpt. Commit labels are evidence only. Internal source structure, package paths, release code, and build metadata cannot set a higher release level without a supported public contract change.
