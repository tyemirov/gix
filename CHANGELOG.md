...
- Added first-class workflow commands for each action (`git branch-prepare`, `git stage`, `git commit`, `git push`, `pull-request create`) plus per-task `steps` configuration, enabling workflows to orchestrate small git/file operations explicitly (see the updated gitignore workflow).
