...
type gitBranchPrepareOperation struct {
...
func buildGitBranchPrepareOperation(options map[string]any) (Operation, error) {
	...
	if !branchExists || len(branch) == 0 {
		return nil, errors.New("git branch-prepare step requires branch name")
	}

	startPoint, _, startErr := reader.stringValue(optionTaskBranchStartPointKeyConstant)
	if startErr != nil {
		return nil, startErr
	}

	ensureClean, ensureCleanExists, ensureCleanErr := reader.boolValue(optionTaskEnsureCleanKeyConstant)
	...
	ensureCleanVariable, _, ensureCleanVarErr := reader.stringValue(optionTaskEnsureCleanVariableKeyConstant)
	if ensureCleanVarErr != nil {
		return nil, ensureCleanVarErr
	}

	return &gitBranchPrepareOperation{
		branchTemplate:      branch,
		startPointTemplate:  startPoint,
		ensureClean:         ensureClean,
		ensureCleanVariable: ensureCleanVariable,
	}, nil
}

func (operation *gitBranchPrepareOperation) Name() string {
	return commandGitBranchPrepareKey
}

func (operation *gitBranchPrepareOperation) Execute(ctx context.Context, environment *Environment, state *State) error {
	variableSnapshot := snapshotVariables(environment)
	return iterateRepositories(state, func(repository *RepositoryState) error {
		taskDef := TaskDefinition{Name: "Branch Prepare"}
		templateData := buildTaskTemplateData(repository, taskDef, variableSnapshot)
		...
		if len(branchName) == 0 {
			return errors.New("branch name resolved to empty value")
		}

		startPoint, startErr := renderTemplateValue(operation.startPointTemplate, repository.Inspection.RemoteDefaultBranch, templateData)
		if startErr != nil {
			return startErr
		}
		...
		plan := taskPlan{
			task: TaskDefinition{
				...
				EnsureCleanVariable: operation.ensureCleanVariable,
			},
			repository:    repository,
			branchName:    branchName,
			startPoint:    startPoint,
			workflowSteps: []workflowAction{branchPrepareAction{branchName: branchName, startPoint: startPoint}},
