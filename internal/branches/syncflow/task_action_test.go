...
func onlyGitHubCommand(t *testing.T, commands []execshell.CommandDetails, action string) execshell.CommandDetails {
	t.Helper()
	var matches []execshell.CommandDetails
	for _, command := range commands {
		if strings.HasPrefix(strings.Join(command.Arguments, " "), action+" ") {
			matches = append(matches, command)
		}
	}
	require.Len(t, matches, 1, "expected one %s operation", action)
	return matches[0]
}
...
func (executor *strictSyncGitExecutor) ExecuteGit(_ context.Context, details execshell.CommandDetails) (execshell.ExecutionResult, error) {
	...
	case gitLSRemoteSubcommandConstant:
		if !commandHasArgument(details.Arguments, "--symref") {
			return execshell.ExecutionResult{}, nil
		}
...
func TestHandleBranchSyncActionStrictPRBranchStagesTrackedPathsMatchedByIgnoreRules(t *testing.T) {
	const commitMessage = "fix: update environment inventory"
	gitExecutor := &strictSyncGitExecutor{
		statusOutput: strings.Join([]string{
			" M configs/.env.hecateapi.example",
	...
	githubClient, githubClientError := githubcli.NewClient(githubExecutor)
	require.NoError(t, githubClientError)
	chatClient := &strictSyncChatClient{response: commitMessage}
	environment := &workflow.Environment{
		GitExecutor:       gitExecutor,
		RepositoryManager: gitManager,
		GitHubClient:      githubClient,
	...
	require.NoError(t, handleBranchSyncAction(context.Background(), environment, repository, parameters))
	recordedCommands := recordedGitCommands(gitExecutor.commands)
	require.Contains(t, recordedCommands, "add --force --all -- configs/.env.hecateapi.example configs/runtime.yml")
	require.Contains(t, recordedCommands, "commit -m "+commitMessage)
	require.Contains(t, recordedCommands, "push origin feature/foo")
	require.NotContains(t, recordedCommands, "check-ignore --stdin")
	require.NotContains(t, recordedCommands, "restore --staged --worktree")
	require.NotContains(t, recordedCommands, "add --all -- configs/.env.local")
	require.Len(t, chatClient.requests, 1)
}
...
func TestHandleBranchSyncActionStrictPRBranchSyncsWithoutPullRequest(t *testing.T) {
	gitExecutor := &strictSyncGitExecutor{}
	gitManager, managerError := gitrepo.NewRepositoryManager(gitExecutor)
	require.NoError(t, managerError)
...
func TestHandleBranchSyncActionStrictPRBranchCreatesMissingRemoteBranchAndPullRequest(t *testing.T) {
	...
	require.Equal(t, []string{"pr", "create", "--repo", "owner/project", "--base", "master", "--head", "feature/foo", "--title", "feature/foo", "--body", pullRequestBody}, onlyGitHubCommand(t, githubExecutor.commands, "pr create").Arguments)
...
func TestHandleBranchSyncActionStrictPRBranchPublishesEmptyLocalMissingRemoteBranch(t *testing.T) {
	gitExecutor := &strictSyncGitExecutor{
		missingReferences: map[string]bool{
			"origin/feature/foo": true,
		},
	...
	require.NoError(t, syncError)
	recordedCommands := recordedGitCommands(gitExecutor.commands)
	require.Contains(t, recordedCommands, "rev-list --count origin/master..feature/foo")
	require.Contains(t, recordedCommands, "merge --no-edit origin/master")
	require.Contains(t, recordedCommands, "push -u origin feature/foo")
	require.Len(t, githubExecutor.commands, 1)
	require.Contains(t, strings.Join(githubExecutor.commands[0].Arguments, " "), "--state merged")
...
func TestHandleBranchSyncActionStrictPRBranchUsesExplicitPullRequestMetadata(t *testing.T) {
	...
	require.Contains(t, recordedGitCommands(githubExecutor.commands), "--state merged")
	require.Equal(t, []string{"pr", "create", "--repo", "owner/project", "--base", "master", "--head", "feature/foo", "--title", "docs: explain sync", "--body", "Explain the reviewer-facing reason."}, onlyGitHubCommand(t, githubExecutor.commands, "pr create").Arguments)
	require.Len(t, chatClient.requests, 1)
}

func TestHandleBranchSyncActionStrictPRBranchCreatesGeneratedBranchFromCurrentDirtyMaster(t *testing.T) {
...
func TestHandleBranchSyncActionStrictPRBranchResolvesGeneratedCurrentDirtyMasterMergeConflict(t *testing.T) {
	...
	gitExecutor := &strictSyncGitExecutor{
		statusOutput:  " M README.md\n",
		revListOutput: "1\n",
		mergeError:    commandFailedError("CONFLICT (content): Merge conflict in README.md"),
		unmergedPaths: "README.md\n",
		conflictStages: map[string]string{
...
func TestHandleBranchSyncActionStrictPRBranchResolvesGeneratedCurrentDirtyMasterDeletionConflict(t *testing.T) {
	generatedBranchName := "gix/remove-obsolete-readme-after-conflict"
	pullRequestBody := "## Summary\n- Keeps the local deletion while merging the current base."
	repositoryPath := t.TempDir()
	readmePath := filepath.Join(repositoryPath, "README.md")
	require.NoError(t, os.WriteFile(readmePath, []byte("remote version left in conflict\n"), 0o644))
	gitExecutor := &strictSyncGitExecutor{
		statusOutput:  " M README.md\n",
		revListOutput: "1\n",
		mergeError:    commandFailedError("CONFLICT (modify/delete): README.md deleted in HEAD and modified in origin/master"),
		unmergedPaths: "README.md\n",
		conflictStages: map[string]string{
			"README.md": "100644 aaaaaaa 1\tREADME.md\n100644 ccccccc 3\tREADME.md\n",
		},
		showOutputs: map[string]string{
			":1:README.md": "base version\n",
			":3:README.md": "remote version\n",
		},
		missingReferences: map[string]bool{
			"origin/" + generatedBranchName:     true,
			"refs/heads/" + generatedBranchName: true,
...
func TestHandleBranchSyncActionStrictPRBranchCreatesGeneratedBranchFromCurrentDirtyMasterWhenLocalBaseAhead(t *testing.T) {
	...
	require.Contains(t, recordedCommands, "add --force --all -- README.md")
...
func TestHandleBranchSyncActionStrictPRBranchSkipsStaleGeneratedRemoteBranchForCurrentDirtyMaster(t *testing.T) {
	generatedBranchName := "gix/cancel-upstream-request-on-downstream-timeout"
	...
	require.Equal(t, []string{"pr", "create", "--repo", "owner/project", "--base", "master", "--head", collisionBranchName, "--title", collisionBranchName, "--body", pullRequestBody}, onlyGitHubCommand(t, githubExecutor.commands, "pr create").Arguments)
	require.Len(t, chatClient.requests, 3)
	require.Contains(t, chatClient.requests[0].Messages[1].Content, "Diff source: ALL")
	require.Contains(t, chatClient.requests[2].Messages[1].Content, "Comparison range: origin/master..."+collisionBranchName)
}

func TestHandleBranchSyncActionStrictPRBranchCommitsDirtyWorkToExplicitMaster(t *testing.T) {
	gitExecutor := &strictSyncGitExecutor{
		statusOutput:  " M README.md\n",
		revListOutput: "1\n",
	}
	...
	githubClient, githubClientError := githubcli.NewClient(githubExecutor)
	require.NoError(t, githubClientError)
	chatClient := &strictSyncChatClient{responses: []string{"docs: preserve dirty work on explicit master"}}
	environment := &workflow.Environment{
		GitExecutor:       gitExecutor,
		RepositoryManager: gitManager,
		GitHubClient:      githubClient,
