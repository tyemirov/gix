...
const (
	...
	commandShortDescriptionConstant = "Pull, merge pending changes, commit, and push the selected branch"
	commandLongDescriptionConstant  = "sync synchronizes a Git branch with its remote. " +
		...
		"Branch names and default-branch status add no other special behavior. Sync pulls the latest remote changes, merges pending work into the selected branch, commits the result, and pushes that branch. The selected branch stays active. " +
		"A GitHub push rejection preserves the local commits and selected branch. Sync reports the rejection, suggests removing branch protection or opening a new pull request, and returns success with exit code 0. " +
		"A missing explicit branch starts at the current HEAD. Pull-request metadata resolves merged parents to their current base for later publication. A rejected parent push preserves child work locally and defers child publication. " +
		...
		"The strict-sync transaction snapshots the caller and target sibling checkout, commit, index, tracked files, untracked files, stashes, and topology, then journals only refs and worktrees it mutates. A local operation failure before publication restores that owned state without rewinding unrelated refs. A GitHub push rejection does not start rollback. An actual remote ref update or pull-request creation marks publication, after which failure preserves the published recovery state and reports a handoff. Invocation-owned stashes are restored with their index and validated before SYNCED is reported. " +
		"When a strict-sync merge conflicts, sync reconstructs untouched bytes locally and directly accepts only cases with no two-sided semantic choice: identical sides and a change on only one side. A text conflict without markers requires an explicit OURS or THEIRS file selection. Binary conflicts stop for explicit Git resolution. Structural conflicts with linked rename paths also require explicit Git resolution. Every marker-bearing region changed by both sides requires semantic LLM audit. Related issue insertions use source selection keys. Gix constructs each selected record from the original bytes, including punctuation and whitespace. Independent issue records remain present. For overlapping insertions, word sequences rank source alternatives. Validation requires an exact source alternative, including punctuation and whitespace. Deletion-intent validation applies at every conflict size. Other concurrent insertions and compatible token edits start from lossless locally derived candidates. Conflicting replacements start from the local alternative plus each compatible incoming edit. Sync sends every derived candidate directly to semantic audit. Candidate generation remains only when local token analysis cannot derive a valid candidate. Each candidate or audit request can exhaust the configured provider order. A provider round with no response stops semantic repair and starts rollback. A locally valid audit correction completes immediately. When exact replacement-intent proof is unavailable for a structurally valid correction, sync retains that exact correction for repair in the next semantic audit. An approval cannot accept that candidate. Only a later locally valid correction completes it. Responses that fail hard validation supply feedback for the next bounded attempt. Rollback occurs only after every safe candidate is exhausted, or after provider failure, cancellation, or an unrecoverable local failure, and always stops before push. " +
		"Sync never rebases or force-pushes. When sync creates a pull request, the body is generated from the branch diff unless --body or sync.pull_request.body supplies explicit text; title defaults to the branch unless --title or sync.pull_request.title supplies it."
