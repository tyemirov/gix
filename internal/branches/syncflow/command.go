...
const (
	...
	commandLongDescriptionConstant  = "sync synchronizes a Git branch with its remote. " +
		...
		"When a strict-sync merge conflicts, sync constructs unchanged regions and independent line changes locally. Identical sides and changes on one side resolve directly. A text conflict without markers requires an explicit OURS or THEIRS file selection. Binary and linked-path conflicts require explicit Git resolution. Overlapping changes require JSON decisions that select exact source bytes or combine compatible changes. Every two-sided candidate requires a separate semantic audit. Read context includes complete small files, affected issue records, or surrounding source lines. The model can request complete file context or report unresolved intent. Each region permits four provider requests across decisions, audits, and context expansion. Repeated rejected candidates and provider failures stop semantic repair. Complete Go, JSON, and YAML files pass syntax checks before staging. Issue trackers reject duplicate identifiers. The existing transaction restores local state before publication when resolution fails. " +
