...
const (
	...
	commandLongDescriptionConstant          = "sync keeps a workspace aligned with the remote-owned master branch and PR-backed work branches. With no branch argument, sync updates the current branch. Existing PR branches sync against their current PR base branch. A missing explicit branch with dirty work is created on top of the current branch. If the current branch is not master, sync first ensures that its committed HEAD is remote-backed and has an open pull request, then opens the child pull request against that branch. The selected parent base is retained for retries after child push or pull-request failure. A clean or stashed missing branch is rejected because it would have no child pull request delta. Dirty work is clustered, described with the configured LLM transport, committed, then pushed through the PR workflow. Dirty auto-commit is rejected on a known-merged branch; use --stash to preserve that work through the merged handoff before creating a new review branch. With master, clean sync restores local master from origin/master; dirty sync creates a PR work branch. Sync never rebases or force-pushes. When sync creates a pull request, the body is generated from the branch diff unless --body or sync.pull_request.body supplies explicit text; title defaults to the branch unless --title or sync.pull_request.title supplies it."
