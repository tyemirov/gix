...
func (service mergeConflictResolutionService) resolvePlannedConflictRegion(ctx context.Context, client llm.ChatClient, options mergeConflictResolutionOptions, file mergeConflictFile, region mergeConflictRegion, index, count int, timeout time.Duration) (string, error) {
	...
	for attempt := 1; attempt <= mergeConflictResolutionMaxSemanticAttempts; attempt++ {
		...
		input := mergePlanRequest{Path: file.Path, Region: region, Phase: phase, Context: readContext, Changes: plan.Changes, Blocks: plan.Blocks, Automatic: plan.Automatic, Candidate: candidate, Feedback: feedback}
